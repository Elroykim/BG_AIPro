...
Jupyter 노트북에서 정확도 95% 찍고 좋아했는데, 막상 프로덕션에 올리려니까 안 돌아가서 멘붕 온 적이 있다. 배포한 뒤에도 시간이 지나니까 성능이 슬슬 떨어지는 것도 문제였다. 이런 문제들을 체계적으로 해결하려고 MLOps를 공부하기 시작했는데, 생각보다 진입장벽이 낮아서 놀랐다.
...
나는 이 조합으로 시작했는데, 소규모 팀에서는 이 정도면 충분하다. [Docker 기초가 부족하다면 AI 개발자를 위한 Docker 가이드]({{< relref "posts/2026-01-14-docker-for-ai-developers.md" >}})를 먼저 읽어보는 걸 추천한다.
...
요즘은 전통적 ML 모델뿐 아니라 LLM 기반 애플리케이션의 운영도 중요해졌다. 실제로 LLM 프로젝트를 운영해보니 프롬프트 버전 관리가 생각보다 골치 아프더라.
