---
title: "파인튜닝 vs RAG: 언제 어떤 걸 써야 할까? (LLM 커스터마이징 비교)"
date: 2026-01-06
...
LLM을 우리 업무에 맞게 쓰려고 알아보면 결국 두 갈래에서 고민하게 된다. 파인튜닝(Fine-tuning)으로 모델 자체를 뜯어고치거나, RAG(Retrieval-Augmented Generation)로 외부 문서를 검색해서 답변에 활용하거나. 처음에는 파인튜닝이 무조건 좋을 줄 알았는데, 직접 해보니 상황에 따라 답이 완전히 달라지더라.

둘 다 써본 경험을 바탕으로 원리, 장단점, 비용까지 정리해봤다.
...
실제로 고객 상담 챗봇 프로젝트에서 하이브리드 방식을 적용해봤는데, 파인튜닝만 했을 때보다 답변 정확도가 눈에 띄게 올랐다.
...
파인튜닝과 RAG는 대립하는 기술이 아니라 상호 보완적인 기술이다. 개인적으로는 대부분의 프로젝트에서 RAG부터 시작하는 걸 추천한다. 비용도 적고 결과를 빠르게 확인할 수 있어서다. RAG 구축이 처음이라면 [RAG 시스템 구축 가이드]({{< relref "posts/2026-01-02-rag-system-guide.md" >}})를 먼저 참고하면 도움이 된다. 스타일이나 형식에 대한 요구사항이 생기면 그때 파인튜닝을 추가하면 된다.
