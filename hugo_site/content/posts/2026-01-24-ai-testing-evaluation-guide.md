...
LLM 기반 서비스를 만들다 보면 제일 답답한 게 테스트다. 기존 소프트웨어는 같은 입력에 같은 결과가 나오니까 assert 하면 끝인데, LLM은 같은 프롬프트에도 매번 다른 응답이 나온다. 처음에 "이걸 어떻게 테스트하지?" 싶어서 꽤 삽질했는데, 그 과정에서 알게 된 방법들을 정리했다.
...
사람이 모든 응답을 평가하는 것은 비용이 큽니다. AI를 평가자로 활용하는 방법인데, 처음에는 "AI가 AI를 평가한다고?" 싶었지만 실제로 해보니 사람 평가와 상관관계가 꽤 높아서 실무에서 유용하게 쓰고 있다.
...
테스트와 배포 파이프라인을 체계적으로 관리하고 싶다면 [MLOps 입문 가이드]({{< relref "posts/2026-01-16-mlops-beginner-guide.md" >}})도 함께 보면 도움이 된다.
