...
"나도 내 챗봇을 만들어볼까?" 싶어서 주말에 한번 도전해봤는데, LLM API 덕분에 생각보다 빨리 돌아가는 물건이 나왔다. 물론 '잘 만든' 챗봇이 되려면 시스템 프롬프트 설계나 메모리 관리 같은 디테일이 중요한데, 삽질하면서 배운 것들을 정리해봤다.

Python으로 AI 챗봇을 처음부터 만들고 웹에 배포하는 과정을 단계별로 다룬다.
...
시스템 프롬프트는 챗봇의 **성격, 역할, 규칙**을 정의합니다. 처음에는 시스템 프롬프트를 대충 쓰고 코드에 시간을 쏟았는데, 실제로는 프롬프트가 챗봇 품질의 80%를 결정한다는 걸 깨달았다.
...
가장 중요한 것은 시스템 프롬프트 설계입니다. 코드는 단순하지만, 프롬프트가 챗봇의 품질을 결정합니다. Claude API의 기능을 더 깊이 활용하고 싶다면 [Claude API 가이드]({{< relref "posts/2025-12-25-claude-api-guide.md" >}})도 함께 참고하세요.
