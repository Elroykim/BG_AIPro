...
클라우드 AI 서비스 구독료가 슬슬 부담되기 시작해서, 로컬에서 LLM을 돌려보자고 마음먹었다. 직접 써보니 DeepSeek-R1이나 Qwen 2.5 같은 오픈소스 모델이 생각보다 훨씬 쓸 만했다. 특히 회사 코드처럼 민감한 데이터를 다룰 때는 로컬이 마음이 편하더라.

이 글에서는 Ollama를 이용해서 내 컴퓨터에서 AI를 무료로 돌리는 방법을 정리한다. 프로그래밍 경험이 없어도 따라 할 수 있다.
...
> **입문자 추천**: 16GB RAM이면 Qwen 2.5 7B로 시작하세요. 한국어 성능이 좋고 가볍습니다. 하드웨어가 여유 있다면 DeepSeek-R1 14B가 최고의 선택입니다. 나는 주로 DeepSeek-R1 14B를 쓰는 편인데, 코딩이랑 한국어 둘 다 잡으려면 이게 가장 밸런스가 좋았다.
...
이 방법을 사용하면, 기존에 OpenAI API로 작성된 프로젝트를 코드 한 줄 변경으로 로컬 LLM으로 전환할 수 있어 매우 유용합니다. 오픈소스 모델을 더 다양하게 탐색하고 싶다면 [Hugging Face 가이드]({{< relref "posts/2026-01-18-hugging-face-guide.md" >}})도 참고해 보세요.
