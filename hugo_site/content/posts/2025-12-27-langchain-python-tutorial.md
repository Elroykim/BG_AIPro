...
LLM API 직접 호출해서 간단한 챗봇 만드는 건 금방인데, "회사 문서를 검색해서 답변하는 챗봇"이나 "여러 도구를 알아서 조합하는 에이전트"를 만들려고 하면 얘기가 달라진다. 처음에는 전부 직접 구현하려다가 코드가 엉망이 됐는데, LangChain을 도입하고 나서 구조가 훨씬 깔끔해졌다.

LangChain은 LLM 기반 앱 개발을 위한 Python 프레임워크로, 2026년 현재 AI 앱 개발의 사실상 표준이다. 입문자 기준으로 핵심 개념을 정리해봤다.
...
위 예시에서 사용한 `|` 연산자가 바로 LCEL입니다. 파이프라인처럼 구성 요소를 연결하는데, 처음 봤을 때 Unix 파이프 느낌이라 금방 익숙해졌다.
...
LangChain은 LLM 기반 애플리케이션을 빠르게 프로토타이핑하고 프로덕션까지 가져갈 수 있는 프레임워크입니다. 이 글에서 다룬 핵심 개념만 이해하면 대부분의 AI 앱을 만들 수 있습니다. RAG 시스템을 더 깊이 파고 싶다면 [RAG 시스템 구축 가이드]({{< relref "posts/2026-01-02-rag-system-guide.md" >}})도 함께 읽어보세요.
