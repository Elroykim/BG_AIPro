---
title: "RAG 시스템 구축 입문: 나만의 AI 지식 베이스 만들기 (Python 가이드)"
date: 2026-01-02
...
사내 문서 기반으로 답변하는 AI를 만들고 싶어서 LLM한테 시켜봤는데, 그럴듯하게 **완전히 틀린 답**을 내놓는 걸 보고 현타가 왔다. 이른바 환각(Hallucination) 문제인데, RAG(Retrieval-Augmented Generation)를 적용하니까 드라마틱하게 개선되더라. 요즘 기업용 AI 앱에서 RAG는 거의 기본 아키텍처가 됐다.

직접 삽질하면서 정리한 내용이라 복사-붙여넣기로 바로 실행 가능한 전체 코드도 포함했다. 따라 하다 보면 나만의 RAG 시스템을 하나 완성할 수 있다.
...
**4단계 - 벡터DB 저장**: 생성된 벡터들을 벡터 데이터베이스에 저장합니다. ChromaDB, Pinecone, Weaviate, Milvus 등 다양한 옵션이 있으며, 각 DB의 특성과 선택 기준은 [벡터 데이터베이스 비교 가이드]({{< relref "posts/2026-01-04-vector-database-comparison.md" >}})에서 자세히 다뤘다.
...
**무료 + 로컬 실행이 필요한 경우**:
- 일반적 용도라면 `intfloat/multilingual-e5-large`를 추천합니다. 한국어 성능이 안정적이고 커뮤니티 지원이 풍부합니다. 개인적으로 여러 모델을 비교해봤는데, 처음 시작한다면 이 모델이 가성비가 제일 좋았다.
...
**상용 서비스 구축 시**:
- `Upstage/solar-embedding-1-large`는 요즘 한국어 임베딩 벤치마크에서 최상위권 성능을 기록하고 있습니다. API 비용이 발생하지만 품질이 우수합니다.
...
오버랩(overlap)은 인접 청크 사이에 겹치는 부분을 두어 문맥이 끊기는 것을 방지합니다. 일반적으로 청크 크기의 10~20% 정도가 적절합니다. 실제로 사내 기술 문서에 적용해봤을 때 청크 500자 + 오버랩 80자 조합이 가장 무난한 결과를 줬다.
