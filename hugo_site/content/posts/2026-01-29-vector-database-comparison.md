...
RAG 시스템을 구축하려면 반드시 필요한 것이 **벡터 데이터베이스**입니다. 텍스트를 벡터(숫자 배열)로 변환한 후 유사도 검색을 수행하는 데이터베이스로, 2026년 현재 AI 애플리케이션의 핵심 인프라로 자리잡았습니다.

하지만 벡터 DB의 선택지가 너무 많습니다. Pinecone, ChromaDB, Weaviate, Milvus, Qdrant, pgvector... 이 글에서는 가장 많이 사용되는 4종을 비교하여 프로젝트에 맞는 최적의 선택을 돕겠습니다.
...
ChromaDB는 로컬에서 바로 실행할 수 있는 임베디드 벡터 DB입니다. SQLite처럼 별도 서버 없이 파이썬 코드 안에서 바로 동작합니다.
...
벡터 데이터베이스 선택은 프로젝트의 규모, 예산, 기술 수준에 따라 달라집니다. 대부분의 경우 **ChromaDB로 시작하여** 프로토타입을 빠르게 만들고, 규모가 커지면 Pinecone이나 Weaviate로 마이그레이션하는 전략이 효과적입니다.
