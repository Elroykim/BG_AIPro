...
요즘 AI 에이전트 이야기가 안 나오는 곳이 없다. Claude Code를 쓰면서 "이거 내가 직접 만들 수 있을까?" 싶어서 파보기 시작했는데, 생각보다 구조가 단순해서 놀랐다. 핵심은 LLM + 도구 + 루프, 이 세 가지다.

Python으로 웹 검색이 되는 리서치 에이전트를 직접 만들어보면서, ReAct 패턴부터 도구 호출, 메모리 관리까지 정리해봤다. 프로그래밍 기초만 있으면 충분히 따라할 수 있다.
...
`description`이 특히 중요합니다. LLM은 이 설명을 읽고 언제 이 도구를 사용할지 판단하기 때문입니다. 설명이 모호하면 LLM이 적절한 시점에 도구를 호출하지 못할 수 있습니다. 처음에 description을 대충 썼다가 에이전트가 엉뚱한 도구를 호출하길래, 설명을 구체적으로 고쳤더니 바로 잘 동작하더라.
...
실전에서는 이 세 가지 전략을 상황에 맞게 조합합니다. 나는 주로 슬라이딩 윈도우 + 도구 결과 압축 조합을 쓰는 편인데, 간단하면서도 효과가 괜찮다.
...
멀티 에이전트 시스템은 2026년 현재 가장 활발히 연구되는 분야입니다. Anthropic의 Claude는 **MCP(Model Context Protocol)**를 통해 에이전트 간 통신을 표준화하고 있습니다. MCP에 대한 자세한 내용은 [MCP 가이드]({{< relref "posts/2026-01-10-mcp-guide-2026.md" >}})에서 다루고 있다.
